...
type Config struct {
	...
	SlotExclude    []string // exact names or glob patterns to exclude
}
...
func ParseConfig() (*Config, error) {
	...
	flag.StringVar(&slotExcludeRaw, "slot-exclude", envOrDefault("PG_SLOT_EXCLUDE", ""), "Slot names to exclude (comma-separated, glob patterns allowed)")
	...
	for _, s := range strings.Split(slotExcludeRaw, ",") {
		s = strings.TrimSpace(s)
		if s != "" {
			cfg.SlotExclude = append(cfg.SlotExclude, s)
		}
	}
